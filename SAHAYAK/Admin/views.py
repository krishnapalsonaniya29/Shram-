...
def seed_super_admin(request):
    # 1. Safety switch
    if os.getenv("ADMIN_SEED_ENABLED") != "true":
        return Response(
            {"message": "Admin seeding disabled"},
            status=status.HTTP_403_FORBIDDEN
        )

    # 2. Token check
    seed_token = request.headers.get("X-ADMIN-SEED-TOKEN")
    if seed_token != os.getenv("ADMIN_SEED_TOKEN"):
        return Response(
    ...
    if not all([username, email, password]):
        return Response(
            {"message": "Missing admin env vars"},
